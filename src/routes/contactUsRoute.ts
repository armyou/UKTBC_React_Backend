...
import { fileUploadMiddleware, cleanupFile } from "../middleware/fileUploadMiddleware";
import fs from "fs";
...
router.post("/submit", fileUploadMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const cleanSubject = subject === "undefined" || subject === undefined ? "No Subject" : subject;
    ...
    console.log(`Sending email to: ${recipientEmail} for category: ${category}`);

    // Generate HTML content
    console.log("data from contact us route: ",contactData);
    const htmlContent = contactEmailService.generateContactEmailHTML(contactData);
...
router.post("/mail", fileUploadMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    // Clean up subject field if it's "undefined" string
    const cleanSubject = subject === "undefined" || subject === undefined ? "Test Contact Form" : subject;
    ...
    const testData = {
      ...
      message: message || "This is a test message for the contact form email service.",
      fileName: req.file ? req.file.originalname : null,
      filePath: req.file ? req.file.path : null
    };

    const htmlContent = contactEmailService.generateContactEmailHTML(testData);
    const recipientEmail = contactEmailService.getRecipientEmail(testData.category);
